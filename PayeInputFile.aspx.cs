...
public partial class PayeInputFile : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
           ...
           // SqlDataAdapter Adp = new SqlDataAdapter("select *,'' as title,'' as nationality,'0' as AnnualUtility,'0' as AnnualMeal, (basic+rent+trans+ltg+others+pension) as AnnualGross,(case when AssessmentAmount=0 then 'No' else 'Yes' end)  as Status from vw_InputFile where TaxMonth='January'", con);
            ...
            SqlDataAdapter Adp = new SqlDataAdapter("select * from vw_InputFile_MainView", con);
    ...
    protected void btn_search_Click(object sender, EventArgs e)
    {
        ...
        if (txt_tax_payer_RIN.Text != "")
            dt_v.RowFilter = "BusinessRIN like '%" + txt_tax_payer_RIN.Text + "%'";

        if (txt_tax_payer_name.Text != "")
            dt_v.RowFilter = "BusinessName like '%" + txt_tax_payer_name.Text + "%'";
